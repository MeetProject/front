    ...
    <>
      {permission[type] !== 'prompt' && (
        <div className='relative flex items-center'>
          <button
            className={`relative flex items-center justify-center border border-solid shadow-sm ${!deviceEnable[type] && permission[type] === 'granted' ? 'border-[#EA4335] bg-[#EA4335]' : 'border-white'} size-14 rounded-full max-[400px]:size-10`}
            type='button'
            onClick={handleButtonClick}
          >
            <IconComponent className='max-[400px]:size-5' fill='#ffffff' height={24} width={24} />
          </button>
          {permission[type] === 'denied' && (
            <div className='absolute top-[10%] right-[18%] flex size-4 items-center justify-center rounded-full bg-[rgb(251,188,4)] max-[400px]:size-3'>
